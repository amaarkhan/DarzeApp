    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.customer == null
              ? (isUrdu ? 'نیا کسٹمر' : 'Add New Customer')
              : (isUrdu ? 'کسٹمر میں ترمیم' : 'Edit Customer'),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            _buildSectionTitle(isUrdu ? 'کسٹمر معلومات' : 'Customer Information'),
            const SizedBox(height: 16),
            _buildTextField(nameController, isUrdu ? 'کسٹمر کا نام' : 'Customer Name', Icons.person),
            const SizedBox(height: 12),
            _buildTextField(phoneController, isUrdu ? 'فون نمبر' : 'Phone Number', Icons.phone, TextInputType.phone),
            const SizedBox(height: 12),
            _buildTextField(colorController, isUrdu ? 'کپڑے کا رنگ' : 'Cloth Color', Icons.palette),
            
            const SizedBox(height: 32),
            
            _buildSectionTitle(isUrdu ? 'کپڑے کی تفصیل' : 'Clothing Details'),
            const SizedBox(height: 16),
            _buildDropdownField(
              selectedClothType,
              [
              ...
              isUrdu ? 'کپڑے کی قسم' : 'Clothing Type',
              (value) {
                setState(() {
            ...
            _buildSectionTitle(isUrdu ? 'پیمائشیں (انچ میں)' : 'Measurements (in inches)'),
            const SizedBox(height: 16),
            _buildMeasurementFields(),
            
            const SizedBox(height: 24),
            
            _buildSectionTitle(isUrdu ? 'اضافی معلومات' : 'Additional Options'),
            const SizedBox(height: 16),
            _buildDropdownField(
              selectedCollarType,
              ['Round', 'Chinese', 'V-Neck', 'Formal'],
              isUrdu ? 'یقہ کی قسم' : 'Collar Type',
              (value) {
                setState(() {
            ...
            _buildDropdownField(
              selectedPocketStyle,
              ['Standard', 'Extra', 'None'],
              isUrdu ? 'جیب کا انداز' : 'Pocket Style',
              (value) {
                setState(() {
            ...
            const SizedBox(height: 16),
            _buildTextField(notesController, isUrdu ? 'اضافی نوٹس' : 'Additional Notes', Icons.note, TextInputType.multiline),
